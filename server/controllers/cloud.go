...
func (c *CloudPlatformController) Modify()  {
	if c.Ctx.Input.IsPost() {
		...
		valid := &validation.Validation{}
		if err := c.ParseForm(form); err == nil {
			fmt.Println("forms:", form)
			if ok, err := valid.Valid(form); err != nil {
				valid.SetError("error", err.Error())
				json["result"] = valid.Errors
			} else if ok {
				user, err := models.DefaultCloudPlatformManager.Modify(form.Id, form.Name, form.Region, form.Remark)
				if err == nil {
					json = map[string]interface{}{
						"code":   200,
						"text":   "更新成功",
		...
		c.Data["info"] = models.DefaultCloudPlatformManager.GetById(pk)
	}
}
...
func (c *CloudPlatformController) Enable()  {
	pk, _ := c.GetInt("pk")
	if err := models.DefaultCloudPlatformManager.Enable(pk); err == nil{
		c.Data["json"] = map[string]interface{} {
			"code":   200,
			"text":   "云账号启用成功",
			"result": nil, //可以返回删除的用户
