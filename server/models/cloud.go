...
func (m *CloudPlatformManager) GetById(id int) *CloudPlatform {
	ormer := orm.NewOrm()
	result := &CloudPlatform{}
	err := ormer.QueryTable(result).Filter("Id__exact", id).Filter("DeletedTime__isnull", true).One(result)
	if err == nil {
		return result
	}
	return nil
...
func (m *CloudPlatformManager) Modify(id int, name, region, remark string) (*CloudPlatform, error) {
	ormer := orm.NewOrm()
	if cloud := m.GetById(id); cloud != nil {
		cloud.Name = name
		cloud.Remark = remark
		cloud.Region = region
		if _, err := ormer.Update(cloud); err != nil {
			return nil, err
		}
		return cloud, nil
	}
...
func (m *CloudPlatformManager) Disable(id int) error {
	ormer := orm.NewOrm()
	cloud := m.GetById(id)
	cloud.Status = 0
	if _, err := ormer.Update(cloud); err != nil {
...
func (m *CloudPlatformManager) Enable(id int) error {
	ormer := orm.NewOrm()
	cloud := m.GetById(id)
	cloud.Status = 1
	if _, err := ormer.Update(cloud); err != nil {
